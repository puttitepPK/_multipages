...
function Navber({ products, carts, setToken, role }) {
  ...
  return (
    <div className="navber-container">
      ...
      {(role === "admin" || role === "boss") && (                    //ถ้าเป็น admin จะเห็น calcu ถ้านอกเหนือจาก role นี้จะไม่เห็น
      <Link to="/calculator">
        <button
          style={{ boxShadow: "0 0 0.25rem gray" }}
          className={
      ...
      onClick={() => {
        ...
        }}>Logout&nbsp; <span className="bi bi-box-arrow-right"></span></button>
    </div>
  );
}
